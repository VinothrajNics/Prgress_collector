...
async function createSession(db, role, clientId) {
...
async function getSession(db, token) {
  ...
  return {
    role: result.role,
    clientId: result.client_id !== null ? Number(result.client_id) : void 0,
...
async function requireClient(c, next) {
  const user = await getAuthUser(c);
  if (!user) {
    return c.json(
      {
...
var SESSION_DAYS, PASSWORD_ITERATIONS;
var init_auth = __esm({
  "src/auth.ts"() {
    "use strict";
    init_modules_watch_stub();
...
// .wrangler/tmp/bundle-fm3td6/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-fm3td6/middleware-insertion-facade.js
init_modules_watch_stub();
...
auth.post("/client-login", async (c) => {
  try {
    ...
    if (!client.password_hash) {
      return c.json(
        {
          error: "Client login has not been configured. Please contact the administrator."
        },
        403
      );
    }
...
clients.get(
  ...
  async (c) => {
    ...
    if (user.role === "client" && user.clientId !== id) {
      return c.json(
        {
          error: "You can only access your own client account"
        },
        403
      );
    }
  ...
  async (c) => {
    ...
    try {
      ...
      if (body.password) {
        ...
        passwordHash = await hashPassword2(
          body.password
        );
      }
...
clients.post(
  ...
  async (c) => {
    ...
    if (!body.department_id || !Number.isInteger(
      Number(
        body.department_id
      )
    )) {
...
var departments_default = departments;

// src/routes/processes.ts
init_modules_watch_stub();
init_auth();
var processes = new Hono2();
processes.get(
  ...
  async (c) => {
    ...
    if (user.role === "client" && user.clientId !== clientId) {
      return c.json(
        {
          error: "You can only access your own client processes"
        },
        403
      );
    }
...
processes.post(
  ...
  async (c) => {
    ...
    const type = body.type === "flow" ? "flow" : "standalone";
    ...
    try {
      const clientDepartment = await c.env.DB.prepare(`
            SELECT
              id
      ...
      const result = await c.env.DB.prepare(`
        ...
        type,
        status,
        type === "flow" ? body.flow_order ?? null : null,
...
processes.put(
  ...
  async (c) => {
    ...
    try {
      ...
      const old = existing;
      const title = body.title !== void 0 ? body.title.trim() : old.title;
      if (!title) {
        return c.json(
          {
      ...
      const flowOrder = type === "flow" ? body.flow_order ?? old.flow_order ?? null : null;
      const description = body.description !== void 0 ? body.description.trim() || null : old.description ?? null;
      const notes = body.notes !== void 0 ? body.notes.trim() || null : old.notes ?? null;
...
var processes_default = processes;
...
admin.get(
  ...
  async (c) => {
    try {
      const clientsRes = await c.env.DB.prepare(`
            SELECT
              id,
              name,
              email,
            ...
            ORDER BY name
          `).all();
      const clients2 = clientsRes.results;
      const result = await Promise.all(
        clients2.map(
          async (client) => {
            const deptsRes = await c.env.DB.prepare(`
                    SELECT
                      cd.id AS client_department_id,
                      d.id AS department_id,
                      d.name,
...
app.get("/", (c) => {
  return c.json({
    status: "ok",
    message: "Collect API running \u{1F680}"
  });
});
