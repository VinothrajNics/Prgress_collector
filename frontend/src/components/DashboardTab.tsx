...
export default function DashboardTab() {
  const { state, mutate, toast, canEdit, showCompany, clientNameOf, scopeClientId } = useApp();
  const ui = useUi();
    ...
    <section className="tab-panel active">
      {showCompany ? (
        <div className="banner">
          Aggregate view across all companies. Use the <b>Company filter</b> (top right) or the{' '}
          <b>Company Management</b> tab to open and edit a company&apos;s workspace.
        </div>
      ) : scopeClientId ? (
        <div className="banner">
          Viewing workspace for <b>{clientNameOf(scopeClientId)}</b>. This maps your organisation&apos;s personal data
          footprint end-to-end &mdash; from group structure down to individual data elements &mdash; and flags DPDP
          compliance gaps automatically.
        </div>
      ) : (
        <div className="banner">
          This workspace maps your organisation&apos;s personal data footprint end-to-end — from group structure down to
          individual data elements — and flags DPDP compliance gaps automatically as you add information.
        </div>
      )}
        ...
        {deptChart.length === 0 ? (
          ...
          deptChart.map(([name, count]) => (
            <div key={name} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              <div style={{ width: 200, fontSize: 12, color: 'var(--navy)', fontWeight: 600 }}>{name}</div>
